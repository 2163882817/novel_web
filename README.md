...
2. **一纲三版**：输入或确认本章细纲后，并行生成 A/B/C 三个正文版本；三版共享剧情主线、关键事件和章末结果，只在叙事表达上进行差异化；
3. **选择与合并**：前端以卡片展示三版，选择任意版本导入主编辑器；未选择的两版继续作为素材面板保留，可复制全文或选中片段粘贴到主编辑器；
4. **开始写作**：也可以使用单版写作，支持 SSE 流式输出；
5. **一致性校对**：检查人物、设定、时间线、伏笔和文风问题；
6. **修订正文**：根据选中的校对问题生成修订稿；
7. **定稿并总结**：生成章节摘要，更新人物状态、伏笔状态和生成日志。

一次“一纲三版”会并行调用模型三次，只有主编辑器中最终确认的内容会保存为正式章节。
...
5. 检查第一卷大纲和人物设定；
6. 进入章节工作台，生成下一章细纲；
7. 输入或调整本章细纲，点击“一纲三版”生成 A/B/C 正文；
8. 对比三张版本卡片，选择一版导入主编辑器；
9. 从另外两版选择需要的片段复制，粘贴到主编辑器合并润色；
10. 保存主编辑器内容为章节；
11. 仔细阅读正文并进行人工编辑；
12. 执行校对，根据报告选择性修订；
13. 确认无误后定稿，让系统更新记忆。
...
| POST | `/api/write/variants` | 根据同一细纲并行生成 A/B/C 三版正文 |
